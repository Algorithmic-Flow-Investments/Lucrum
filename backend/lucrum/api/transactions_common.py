...
from lucrum.models import Transaction, Tag, Category, Budget, Target
...
def transactions_find(model, query: MultiDict):
	...
	account_id = query.get('account_id', None, int)
	...
	if account_id:
		transactions_query = transactions_query.filter(Transaction.account_id == account_id)
	if target_id:
		if target_id == -1:
